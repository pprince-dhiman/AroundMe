...
export const createCulturalEventController = async (req, res) => {
  try {
    ...
    if (pricing.isFree === false && pricing.amount <= 10) {
      return res.json({
        success: false,
        message: "Amount should be more than \u20B910 for paid cultural events",
      });
    }

    const result = await createCulturalEventService({
      body: req.body,
      orgId,
      userId,
    });
    ...
    res.json({
      success: true,
      message: "New Cultural event created successfully.",
      data: result,
    });
  } catch (err) {
    console.log(err);
