        ...
        <div className="flex items-center justify-between mb-8">
          ...
          {events && (
            <button
              ...
              className="hidden md:flex items-center gap-2 text-sm font-medium transition-colors duration-200  border rounded px-3 py-1 hover:bg-gray-700 hover:text-white"
            >
              View All
        ...
        {events ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {popularEvents?.slice(0, 4)?.map((event) => (
              <PopularEventsCard event={event} key={event._id} />
            ))}
          </div>
        ) : (
          <Loading />
