  ...
  return (
    <div className="my-5 ">
      <div className="flex items-center justify-between">
        <h2 className="font-semibold text-lg">Judging Criteria</h2>
        <button
          type="button"
      ...
      <div>
        {judgingCriteriaFields?.map((field, index) => (
          <div
            key={field.id}
            className=" border rounded-xl p-4 grid grid-cols-10 gap-5 my-3"
          >
            <input
              {...register(`judgingCriteria.${index}`)}
              placeholder={`Judging Criteria ${index+1}`}
              className=" w-full border rounded-lg px-3 py-2 border-gray-500 col-span-9"
            />
            {errors.judgingCriteria?.[index] && (
              <div className="text-sm text-red-500">
                {errors.judgingCriteria?.[index]?.message}
              </div>
            )}

            <button
              type="button"
              onClick={() => deleteJudgingCriteria(index)}
